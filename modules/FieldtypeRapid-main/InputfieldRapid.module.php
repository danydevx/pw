 ...
 * Loads a vendored EditorJS bundle — no CDN, no build step for end users.
 * Developers: run `cd js && npm install && npm run build` to update dist/.
 */
class InputfieldRapid extends Inputfield {
	...
	public function ___render(): string {
		...
		// Normalize image blocks: promote file.url → url so EditorJS renders them correctly
		...
		// CSS class for full-width mode (removes EditorJS internal max-width)
	...
	protected function loadAssets(): void {
		...
		$url     = $config->urls->get('FieldtypeRapid');
		$distDir = $config->paths->get('FieldtypeRapid') . 'js/dist/';
		...
		$config->scripts->add($url . "js/dist/editor.js?v=$v");
		$config->styles->add($url  . "js/editor.css?v=$v");
	}
