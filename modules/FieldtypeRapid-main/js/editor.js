/**
 * FieldtypeRapid — editor bundle entry point
	...
	return {
		...
		attaches: {
			...
			config: {
				...
				endpoint: uploadUrl.replace('/upload/', '/attach/') + '?pageId=' + pageId,
				field:    'file',
			},
		},
		code:      { class: CodeTool },
...
function initEditor({ holderId, valueId, data, config }) {
	...
	// EditorJS built-ins (bold, italic, link) are controlled via the tools config with inlineToolbar
