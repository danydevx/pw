 ...
 * RapidRenderer
 *
 * Standalone renderer for EditorJS JSON data.
...
class RapidRenderer {
	...
	public function renderData(array $data, ?Page $page = null, ?Field $field = null): string {
		...
		while ($i < $total) {
			...
			// paragraph is EditorJS built-in default — never shown in the allowed-blocks checklist
	...
	private static function boot(): void {
		...
		// Also try loading from blocks/ in case renderer is used standalone.
		$dir = (self::$moduleDir ?: __DIR__) . '/blocks/';
		if (is_dir($dir)) {
