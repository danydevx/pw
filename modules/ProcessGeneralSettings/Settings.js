$(window).on('load', function() {
                ...
                '<form id="modalForm" class="uk-form-stacked">' +
                    ...
                    '<div class="uk-grid-small uk-child-width-1-2@m" uk-grid>' +
                        ...
                        '<div><label class="uk-form-label" for="type">Type</label><div class="uk-form-controls"><select name="type" id="type" class="uk-select">' +
                            ...
                            '<option>URL</option><option>Email</option><option>Image</option><option>File</option><option>Fieldset</option><option>Markup</option><option>PageListSelect</option>' +
                        '</select></div></div>' +
                        '<div><label class="uk-form-label" for="width">Width in %</label><div class="uk-form-controls"><input type="number" max="100" min="10" name="width" id="width" value="100" class="uk-input"><small class="pgs-help">Range: 10 to 100</small></div></div>' +
    ...
    function validateLabelLive() {
        ...
        var labelValue = $.trim(labelField.val());
    ...
    function parseOptions(selectRaw) {
        ...
        $.each(tokens, function(_, t) {
            ...
            if (token.indexOf(':') > -1) {
                ...
                if (!key) return;
                out.push({ key: key, label: label || key });
            } else {
                var k = sanitizeKey(token);
                if (!k) return;
                out.push({ key: k, label: token });
            }
        });
        return out;
    }
    ...
    $(document).on('input blur', '#width', function() {
